...
把本地 Markdown 转成便于分享的 HTML 或 PDF。默认将本地图片内嵌到单个 HTML，支持代码高亮、表格和按一级标题和分割线分页。
...
也可安装构建好的 wheel：`python -m pip install dist/md2share-0.1.1-py3-none-any.whl`。
...
PDF 默认遇到一级标题或 Markdown 分割线（如 `---`）另起一页；分割线不依赖标题，二者紧邻时不会产生额外空白页。分割线在网页中保留，在 PDF 中隐藏。程序尝试缩小页内图片，长段内容仍可跨页。`--continuous` 关闭标题/分割线分页和图片自动适配。远程图片保留 URL，不会下载内嵌。
...
[示例 Markdown](https://github.com/fenghansen/md2share/blob/main/examples/pagination.md) 演示一级标题分页、分割线独立分页及两者相邻不产生空白页。

```bash
md2share examples/pagination.md --pdf
